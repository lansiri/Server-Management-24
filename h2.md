...
**D**
Run several idempotent (state.single) commands over the master-slave connection.

**E**
Collect technical information about slaves over the network (grains.item)

**F**
Hello, IaC. Make infra as code by typing /srv/salt/hello/init.sls. Drive the modules to a slave. The module can  for example, create an example file in a directory. Test with another command that the change you requested has really been made.
